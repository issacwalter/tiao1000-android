...
public class  AjaxRequestHandler extends  AsyncTask<Object, Object, Object> {
	...
	private void makeRequestWithRetries(HttpUriRequest request) throws ConnectException {
		...
		while (retry) {
			...
			try {
				...
				publishProgress(Update_failure, e,"can't resolve host",response);
				return;
			} catch (IOException e) {
				cause = e;
	...
	protected Object doInBackground(Object... params) {
		...
		try {
			...
			publishProgress(Update_failure,e,null,null); // 结束
		}
	...
	protected void onProgressUpdate(Object... values) {
		...
		case Update_failure:
			if(callback!=null)
				callback.onFailure((Throwable)values[1],(String)values[2],values[3]==null?null:(HttpResponse)values[3]);
			break;
		case Update_success:
			if(callback!=null)
				callback.onSuccess(values[1],values[2]==null?null:(HttpResponse)values[2]);
			break;
		default:
			break;
		}
	...
	private void handleResponse(HttpResponse response) {
		...
		if (status.getStatusCode() >= 300) {
			publishProgress(Update_failure,new HttpResponseException(status.getStatusCode(), status.getReasonPhrase()),
					"response status code:"+status.getStatusCode(),response);
		} else {
			try {
				...
				publishProgress(Update_success,responseBody,response);
				
			} catch (IOException e) {
				publishProgress(Update_failure,e,(String)null,response);
			}
