...
public class FinalHttp {
    ...
    */
    ...
    public void get( String url,String cookiesStr, AjaxParams params,AjaxCallBack<? extends Object> callBack) {
    	HttpUriRequest request = new HttpGet(getUrlWithQueryString(url, params));
    	if(cookiesStr!=null) request.addHeader("Cookie", cookiesStr);
        sendRequest(httpClient, httpContext, request, null, callBack);
    }
    ...
    public void post( String url,String cookiesStr, AjaxParams params,AjaxCallBack<? extends Object> callBack) {
        HttpEntityEnclosingRequestBase request = new HttpPost(url);
        if(params != null) request.setEntity(paramsToEntity(params));
        if(cookiesStr!=null) request.addHeader("Cookie", cookiesStr);
        sendRequest(httpClient, httpContext, request, null, callBack);
  ...
  //------------------put 请求-----------------------
    ...
    protected <T> void sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, AjaxCallBack<T> ajaxCallBack) {
        ...
        uriRequest.addHeader("Referer", "http://www.tiao1000.com");
