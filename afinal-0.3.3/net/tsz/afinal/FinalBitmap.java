...
public class FinalBitmap {
	 ...
	 * @return
	 */
	public FinalBitmap configLoadedListener(OnImageloadedListener loadedListener){
		mConfig.imageLoaded = loadedListener;
		return this;
	}
	
	public FinalBitmap configImageSizeReadyListener(OnImageSizeReady imageSizeReadyListener){
		mConfig.imageSizeReady = imageSizeReadyListener;
		return this;
	}
	...
	private void doDisplay(ImageView imageView, String uri, BitmapDisplayConfig displayConfig) {
		...
		if (bitmap != null) {
			if(this.mConfig.imageSizeReady != null) this.mConfig.imageSizeReady.imageSizeReady(bitmap.getWidth(), bitmap.getHeight(), uri, bitmap, imageView);
			imageView.setImageBitmap(bitmap);
			if(this.mConfig.imageLoaded != null) this.mConfig.imageLoaded.imageLoadComplete(uri, bitmap, imageView);
    ...
    public void onResume(){
    	...
    	Logger.info("FinalBitmap onResume");
    }
    ...
    public void onPause() {
        ...
        flushCache();
        Logger.info("FinalBitmap onPause");
    }
    ...
    public void onDestroy() {
        closeCache();
        Logger.info("FinalBitmap onDestroy");
    }
	...
	private class BitmapLoadAndDisplayTask extends AsyncTask<Object, Void, Bitmap> {
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			if(mConfig.imageSizeReady != null) mConfig.imageSizeReady.imageSizeReady(bitmap==null?-1:bitmap.getWidth(), bitmap==null?-1:bitmap.getHeight(), String.valueOf(data), bitmap, imageView);
			...
			if(mConfig.imageLoaded != null){
				mConfig.imageLoaded.imageLoadComplete(String.valueOf(data), bitmap,imageView);
			}
		}
	...
	private class FinalBitmapConfig {
		...
		public String cachePath;
		 ...
		 public OnImageloadedListener imageLoaded = null;/*图片加载完成的监听器*/
		 public OnImageSizeReady imageSizeReady = null;/*图片尺寸可知监听器*/
	...
	public interface OnImageloadedListener{
		 ...
		 */
		void imageLoadComplete(String url,Bitmap bitmap,ImageView imageView);
	}
	
	public interface OnImageSizeReady{
		/**
		 * 当图片的尺寸可知的时候
		 * @param width 图片的宽度
		 * @param height 图片的高度
		 * @param url 图片的地址
		 * @param bitmap 图片
		 * @param imgView 要显示到的imageview
		 */
		void imageSizeReady(int width,int height,String url,Bitmap bitmap,ImageView imgView);
	}
}
